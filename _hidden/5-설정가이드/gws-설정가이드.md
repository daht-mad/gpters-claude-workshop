...
> 작성일: 260317 | 대상: macOS (Apple Silicon) / Windows 10+
...
> ⚠️ Google은 **winget / Chocolatey를 공식 지원하지 않습니다.** 공식 인스톨러를 사용하세요.

**방법 A: 공식 인스톨러 다운로드 (권장)**

1. [공식 인스톨러](https://dl.google.com/dl/cloudsdk/channels/rapid/GoogleCloudSDKInstaller.exe) 다운로드 → 실행
2. 설치 중 옵션: Python 3 번들 설치는 **체크 유지** (필수)
3. 설치 완료 후 "Start Google Cloud SDK Shell" 옵션은 체크 해제하고 종료

**방법 B: PowerShell로 한 번에**
...
> 요구사항: Windows 8.1 이상 또는 Windows Server 2012 이상
...
### 방법 A: npm (권장 — macOS/Windows 공통, 가장 간단)

```bash
npm install -g @googleworkspace/cli
```

> Windows에서는 **PowerShell**이나 **명령 프롬프트**에서 실행하세요. 사전에 [Node.js](https://nodejs.org)가 설치되어 있어야 합니다.

### 방법 B: Homebrew (macOS / Linux 전용)
...
> Windows에서는 이 방법을 사용할 수 없습니다 — 방법 A(npm) 또는 방법 D(GitHub Releases)를 사용하세요.
...
### 방법 D: GitHub Releases 바이너리 (Windows 권장 대안)

npm을 쓸 수 없거나 단일 바이너리로 관리하고 싶다면:

1. [GitHub Releases](https://github.com/googleworkspace/cli/releases) 페이지 접속
2. 본인 OS/아키텍처에 맞는 바이너리 다운로드
   - Windows: `gws-x86_64-pc-windows-msvc.zip` (또는 유사한 이름)
   - macOS Apple Silicon: `gws-aarch64-apple-darwin.tar.gz`
3. 압축 해제 후 `gws.exe`(또는 `gws`)를 PATH에 포함된 폴더로 이동
   - Windows: `C:\Users\<사용자>\bin\` 같은 폴더를 만들고 시스템 환경변수 PATH에 추가
   - macOS/Linux: `/usr/local/bin/`이나 `~/.local/bin/`으로 이동
...
> ⚠️ 기존에 `gog`이 `/opt/homebrew/bin/gog`에 설치되어 있다면,
...
PATH에 없으면 시스템 환경변수에 `%APPDATA%\npm` 또는 `npm config get prefix` 결과 경로를 추가하고 터미널을 새로 열어주세요.
...
```
1. GoogleCloudSDKInstaller.exe 다운로드·실행   ← gcloud 설치
2. gcloud init                                ← 브라우저에서 Google 로그인
3. npm install -g @googleworkspace/cli        ← gws 설치
4. gws auth setup                             ← 브라우저에서 OAuth 동의
5. gws gmail messages list                    ← 동작 확인!
